...
namespace PiecesOfCode
{
	const std::string TArray_Iterator =
		R"cpp(template<typename TArray>
class TIterator
{
public:
...
public:
	...
	const std::string TArray_Class =
		R"cpp(template<typename InElementType>
class TArray
{
public:
		...
		R"cpp(class UFunction* UFunction::FindFunction(const std::string& functionFullName)
{
	...
	const std::string EEnumFlags =
		R"cpp(// https://github.com/CodeRedModding/UnrealEngine3/blob/main/Development/Src/Core/Inc/UnStack.h#L48
// State Flags
enum EStateFlags
{
